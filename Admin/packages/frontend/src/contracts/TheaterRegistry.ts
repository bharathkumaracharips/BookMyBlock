...
export class AdminTheaterRegistryService {
    ...
    async initialize(): Promise<boolean> {
        try {
            ...
            const accounts = await (this.provider as any).listAccounts()
            if (accounts.length === 0) {
                throw new Error('No accounts available in Ganache')
            }

            this.signer = await (this.provider as any).getSigner(accounts[0].address)
    ...
    // Get user applications by UID
    async getUserApplications(uid: string): Promise<BlockchainApplication[]> {
        if (!this.contract) {
            throw new Error('Contract not initialized')
        }

        try {
            console.log('📡 Admin fetching applications for UID:', uid)
            const applications = await this.contract.getUserApplications(uid)
            console.log('📡 Admin found applications:', applications.length)

            return applications.map((app: any, index: number) => {
                console.log(`📊 Admin Application ${index + 1}:`, {
                    uid: app.uid,
                    status: app.status,
                    statusNumber: Number(app.status),
                    ipfsHash: app.ipfsHash,
                    lastUpdated: new Date(Number(app.lastUpdated) * 1000).toLocaleString()
                })

                return {
                    applicationId: `APP_${index + 1}`, // Generate consistent ID
                    uid: app.uid,
            ...
            console.error('❌ Admin error getting user applications:', error)
            return []
        }
    }
