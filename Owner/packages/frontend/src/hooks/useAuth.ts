...
export function useAuth() {
  ...
  useEffect(() => {
    if (ready && authenticated && user && wallets.length === 0) {
      console.log('🏢 Owner: No wallets found, creating embedded wallet...')
      createWallet().catch(error => {
        console.error('🏢 Owner: Failed to create wallet:', error)
      })
    }
  ...
  const apiCall = async (endpoint: string, options: RequestInit = {}) => {
    try {
      ...
      return fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8002/api'}${endpoint}`, {
        ...options,
        headers: {
